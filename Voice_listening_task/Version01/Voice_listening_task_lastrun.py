...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='E:\\post_log\\voice_imagery\\exp\\exp_preparation\\Voice imagery task\\Program_psychopy\\Voice_listening_task\\test\\Sub01\\Voice_listening_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisFormal1_1_loop_trial in formal1_1_loop_trials:
        ...
        while continueRoutine and routineTimer.getTime() < 3.86:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if content_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > content_text.tStartRefresh + 3.86-frameTolerance:
                    # keep track of stop time/frame for later
