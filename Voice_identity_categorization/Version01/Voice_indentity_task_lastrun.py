...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='E:\\post_log\\voice_imagery\\exp\\exp_preparation\\Voice identity categorization task\\Program_psychopy\\Voice_identity_categorization\\2024\\Voice_indentity_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instr_practice_image = visual.ImageStim(
        ...
        image='image/instr_practice.jpg', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.78, 1),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    ...
    rest_image = visual.ImageStim(
        ...
        ori=0.0, pos=(0, 0), draggable=False, size=(1.78, 7),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    ...
    for thisPractice_loop_trial in practice_loop_trials:
        ...
        sound_practice.setSound(currentpracticefilepath, secs=1.5, hamming=True)
        ...
        while continueRoutine and routineTimer.getTime() < 1.5:
            ...
            if sound_practice.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_practice.tStartRefresh + 1.5-frameTolerance or sound_practice.isFinished:
                    # keep track of stop time/frame for later
        ...
        selection_practice = data.Routine(
            ...
            components=[option1_practice_text, option2_practice_text, option3_practice_text, selection_practice_key_resp],
        )
        ...
        # 随机分配三个身份到三个按键位置
        voice_types = ['familiar', 'celebrity', 'unfamiliar']
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if selection_practice_key_resp.status == STARTED and not waitOnFlip:
                theseKeys = selection_practice_key_resp.getKeys(keyList=['1','2','3'], ignoreKeys=["escape"], waitRelease=False)
    ...
    while continueRoutine:
        ...
        # if instr_formal1_image is stopping this frame...
        if instr_formal1_image.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instr_formal1_image.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                instr_formal1_image.tStop = t  # not accounting for scr refresh
                instr_formal1_image.tStopRefresh = tThisFlipGlobal  # on global time
                instr_formal1_image.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instr_formal1_image.stopped')
                # update status
                instr_formal1_image.status = FINISHED
                instr_formal1_image.setAutoDraw(False)
    ...
    for thisFormal1_1_loop_trial in formal1_1_loop_trials:
        ...
        thisExp.addData('correct_answer', thisExp.correctAnswer)
        formal_1_sound.setSound(currentfilepath, secs=1.5, hamming=True)
        ...
        while continueRoutine and routineTimer.getTime() < 1.5:
            ...
            if formal_1_sound.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > formal_1_sound.tStartRefresh + 1.5-frameTolerance or formal_1_sound.isFinished:
                    # keep track of stop time/frame for later
        ...
        selection_formal = data.Routine(
            ...
            components=[option1_text, option2_text, option3_text, selection_key_resp],
        )
