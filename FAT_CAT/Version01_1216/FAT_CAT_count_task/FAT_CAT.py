...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\YULAB\\Downloads\\FAT_CAT_count_task (1)\\FAT_CAT_count_task\\Voice_indentity_task.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    deviceManager.addDevice(
        deviceClass='psychopy.hardware.microphone.MicrophoneDevice',
        deviceName='controlled_response_free_mic',
        index=5,
        maxRecordingSize=24000.0,
        channels=None, 
        sampleRateHz=48000, 
    ...
    deviceManager.addDevice(
        deviceClass='psychopy.hardware.microphone.MicrophoneDevice',
        deviceName='after_controlled_response_free_mic',
        index=5,
        maxRecordingSize=24000.0,
        channels=None, 
        sampleRateHz=48000, 
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Make folder to store recordings from free_response_mic
    free_response_micRecFolder = filename + '_free_response_mic_recorded'
    if not os.path.isdir(free_response_micRecFolder):
        os.mkdir(free_response_micRecFolder)
    # Make folder to store recordings from controlled_response_free_mic
    controlled_response_free_micRecFolder = filename + '_controlled_response_free_mic_recorded'
    if not os.path.isdir(controlled_response_free_micRecFolder):
        os.mkdir(controlled_response_free_micRecFolder)
    # Make folder to store recordings from after_controlled_response_free_mic
    after_controlled_response_free_micRecFolder = filename + '_after_controlled_response_free_mic_recorded'
    if not os.path.isdir(after_controlled_response_free_micRecFolder):
        os.mkdir(after_controlled_response_free_micRecFolder)
    ...
    stimulus_file = os.path.join(os.getcwd(), 'stimulus', 'Stimulus60.xlsx')
    ...
    try:
        ...
        stimulus_df = pd.DataFrame({
            'wordID': range(1, 61),  
            'word_list': [f'句子_{i}' for i in range(1, 61)],
            'word_category': ['Nature', 'Objects', 'States', 'People', 'Animals/Plants', 'Actions'] * 10
        })
        category_df = pd.DataFrame({
            'word_category': ['Nature', 'Objects', 'States', 'People', 'Animals/Plants', 'Actions'],
            'Instructions': [f'{cat}类别的指导语' for cat in ['Nature', 'Objects', 'States', 'People', 'Animals/Plants', 'Actions']]
        })
    ...
    def create_stimulus_conditions(stimulus_df, category_df):
        ...
        # 检查数据中是否包含必要的列
        if 'word_category' not in stimulus_df.columns:
            print("错误：刺激材料数据框中缺少'word_category'列")
            return None
        ...
        for category in categories:
            # 获取当前类别的所有刺激
            category_stimuli = stimulus_df[stimulus_df['word_category'] == category]
            
            # 检查当前类别是否有足够的刺激材料
            if len(category_stimuli) < 2:
                print(f"警告：类别 '{category}' 只有 {len(category_stimuli)} 个刺激，需要至少2个")
                continue
            
            # 随机选择2个不同的刺激
            selected_indices = random.sample(list(category_stimuli.index), 2)
            stim1 = category_stimuli.loc[selected_indices[0]].copy()
            stim2 = category_stimuli.loc[selected_indices[1]].copy()
            
            # 随机决定哪个是free，哪个是controlled
            if random.random() > 0.5:
                stim1['condition'] = 'free'
                stim2['condition'] = 'controlled'
            else:
                stim1['condition'] = 'controlled'
                stim2['condition'] = 'free'
            
            selected_stimuli.extend([stim1, stim2])
        
        # 创建结果DataFrame
        result_df = pd.DataFrame(selected_stimuli)
        for idx, row in result_df.iterrows():
            other_categories = [cat for cat in categories if cat != row['word_category']]
        # 随机选择一个类别作为controlled_category
            result_df.at[idx, 'controlled_category'] = random.choice(other_categories)
    ...
    free_association_text = visual.TextStim(win=win, name='free_association_text',
        ...
        languageStyle='LTR',
        depth=-2.0);
    # make microphone object for free_response_mic
    free_response_mic = sound.microphone.Microphone(
        device='free_response_mic',
        name='free_response_mic',
        recordingFolder=free_response_micRecFolder,
        recordingExt='wav'
    )
    ...
    Control_count_text = visual.TextStim(win=win, name='Control_count_text',
        ...
        languageStyle='LTR',
        depth=-2.0);
    # make microphone object for controlled_response_free_mic
    controlled_response_free_mic = sound.microphone.Microphone(
        device='controlled_response_free_mic',
        name='controlled_response_free_mic',
        recordingFolder=controlled_response_free_micRecFolder,
        recordingExt='wav'
    )
    ...
    Control_count_text_2 = visual.TextStim(win=win, name='Control_count_text_2',
        ...
        languageStyle='LTR',
        depth=-1.0);
    # make microphone object for after_controlled_response_free_mic
    after_controlled_response_free_mic = sound.microphone.Microphone(
        device='after_controlled_response_free_mic',
        name='after_controlled_response_free_mic',
        recordingFolder=after_controlled_response_free_micRecFolder,
        recordingExt='wav'
    )
    ...
    # completed 8.0 repeats of 'instruction_loop'
    ...
    for thisTrials_loop in trials_loop:
        ...
        free_response_talk = data.Routine(
            ...
            components=[count_text, free_association_text, free_response_mic, count_key_resp, free_response_key_resp],
        )
        ...
        while continueRoutine:
            ...
            # if free_response_mic is starting this frame...
            if free_response_mic.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                free_response_mic.frameNStart = frameN  # exact frame index
                free_response_mic.tStart = t  # local t and not account for scr refresh
                free_response_mic.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(free_response_mic, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.addData('free_response_mic.started', t)
                # update status
                free_response_mic.status = STARTED
                # start recording with free_response_mic
                free_response_mic.start()
            
            # if free_response_mic is active this frame...
            if free_response_mic.status == STARTED:
                # update params
                pass
                # update recorded clip for free_response_mic
                free_response_mic.poll()
            
            # if free_response_mic is stopping this frame...
            if free_response_mic.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > free_response_mic.tStartRefresh + 180-frameTolerance:
                    # keep track of stop time/frame for later
                    free_response_mic.tStop = t  # not accounting for scr refresh
                    free_response_mic.tStopRefresh = tThisFlipGlobal  # on global time
                    free_response_mic.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('free_response_mic.stopped', t)
                    # update status
                    free_response_mic.status = FINISHED
                    # stop recording with free_response_mic
                    free_response_mic.stop()
        ...
        # tell mic to keep hold of current recording in free_response_mic.clips and transcript (if applicable) in free_response_mic.scripts
        # this will also update free_response_mic.lastClip and free_response_mic.lastScript
        free_response_mic.stop()
        tag = data.utils.getDateStr()
        free_response_micClip = free_response_mic.bank(
            tag=tag, transcribe='None',
            config=None
        )
        trials_loop.addData(
            'free_response_mic.clip', free_response_mic.recordingFolder / free_response_mic.getClipFilename(tag)
        )
        # check responses
        ...
        Controlled_response_talk = data.Routine(
            ...
            components=[Controlled_association_text_free, Control_count_text, controlled_response_free_mic, Control_count_key_resp, controlled_response_key_resp],
        )
        ...
        while continueRoutine:
            ...
            # if controlled_response_free_mic is starting this frame...
            if controlled_response_free_mic.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                controlled_response_free_mic.frameNStart = frameN  # exact frame index
                controlled_response_free_mic.tStart = t  # local t and not account for scr refresh
                controlled_response_free_mic.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(controlled_response_free_mic, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.addData('controlled_response_free_mic.started', t)
                # update status
                controlled_response_free_mic.status = STARTED
                # start recording with controlled_response_free_mic
                controlled_response_free_mic.start()
            
            # if controlled_response_free_mic is active this frame...
            if controlled_response_free_mic.status == STARTED:
                # update params
                pass
                # update recorded clip for controlled_response_free_mic
                controlled_response_free_mic.poll()
            
            # if controlled_response_free_mic is stopping this frame...
            if controlled_response_free_mic.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > controlled_response_free_mic.tStartRefresh + 180-frameTolerance:
                    # keep track of stop time/frame for later
                    controlled_response_free_mic.tStop = t  # not accounting for scr refresh
                    controlled_response_free_mic.tStopRefresh = tThisFlipGlobal  # on global time
                    controlled_response_free_mic.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('controlled_response_free_mic.stopped', t)
                    # update status
                    controlled_response_free_mic.status = FINISHED
                    # stop recording with controlled_response_free_mic
                    controlled_response_free_mic.stop()
        ...
        # tell mic to keep hold of current recording in controlled_response_free_mic.clips and transcript (if applicable) in controlled_response_free_mic.scripts
        # this will also update controlled_response_free_mic.lastClip and controlled_response_free_mic.lastScript
        controlled_response_free_mic.stop()
        tag = data.utils.getDateStr()
        controlled_response_free_micClip = controlled_response_free_mic.bank(
            tag=tag, transcribe='None',
            config=None
        )
        trials_loop.addData(
            'controlled_response_free_mic.clip', controlled_response_free_mic.recordingFolder / controlled_response_free_mic.getClipFilename(tag)
        )
        # check responses
        ...
        after_control_response = data.Routine(
            ...
            components=[Control_count_text_2, after_controlled_response_free_mic, Controlled_association_text_free_4, Control_count_key_resp_2, controlled_response_key_resp_2],
        )
        ...
        while continueRoutine:
            ...
            # if after_controlled_response_free_mic is starting this frame...
            if after_controlled_response_free_mic.status == NOT_STARTED and t >= 0-frameTolerance:
                # keep track of start time/frame for later
                after_controlled_response_free_mic.frameNStart = frameN  # exact frame index
                after_controlled_response_free_mic.tStart = t  # local t and not account for scr refresh
                after_controlled_response_free_mic.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(after_controlled_response_free_mic, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.addData('after_controlled_response_free_mic.started', t)
                # update status
                after_controlled_response_free_mic.status = STARTED
                # start recording with after_controlled_response_free_mic
                after_controlled_response_free_mic.start()
            
            # if after_controlled_response_free_mic is active this frame...
            if after_controlled_response_free_mic.status == STARTED:
                # update params
                pass
                # update recorded clip for after_controlled_response_free_mic
                after_controlled_response_free_mic.poll()
            
            # if after_controlled_response_free_mic is stopping this frame...
            if after_controlled_response_free_mic.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > after_controlled_response_free_mic.tStartRefresh + 180-frameTolerance:
                    # keep track of stop time/frame for later
                    after_controlled_response_free_mic.tStop = t  # not accounting for scr refresh
                    after_controlled_response_free_mic.tStopRefresh = tThisFlipGlobal  # on global time
                    after_controlled_response_free_mic.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('after_controlled_response_free_mic.stopped', t)
                    # update status
                    after_controlled_response_free_mic.status = FINISHED
                    # stop recording with after_controlled_response_free_mic
                    after_controlled_response_free_mic.stop()
        ...
        # tell mic to keep hold of current recording in after_controlled_response_free_mic.clips and transcript (if applicable) in after_controlled_response_free_mic.scripts
        # this will also update after_controlled_response_free_mic.lastClip and after_controlled_response_free_mic.lastScript
        after_controlled_response_free_mic.stop()
        tag = data.utils.getDateStr()
        after_controlled_response_free_micClip = after_controlled_response_free_mic.bank(
            tag=tag, transcribe='None',
            config=None
        )
        trials_loop.addData(
            'after_controlled_response_free_mic.clip', after_controlled_response_free_mic.recordingFolder / after_controlled_response_free_mic.getClipFilename(tag)
        )
        # check responses
    ...
    # completed 12.0 repeats of 'trials_loop'
    
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    # save free_response_mic recordings
    free_response_mic.saveClips()
    # save controlled_response_free_mic recordings
    controlled_response_free_mic.saveClips()
    # save after_controlled_response_free_mic recordings
    after_controlled_response_free_mic.saveClips()
